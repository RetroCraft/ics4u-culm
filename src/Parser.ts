...
export class Parser {
   ...
   */
  private tree: { [key: string]: Token } = {};
  /**
   * List of primitives
   */
  private primitives: { [key: string]: Primitive } = {};
  /**
   * Tree as array for ease of manipulation
   */
  private get treeVals(): Token[] {
  ...
  public test() {
    for (const key in this.tree) {
      ...
      console.assert(
        key === String(eval(this.tree[key].toString())),
        `Token ${key} should evaluate to itself`
      );
    }
  }
  ...
  private addToken(
    ...
    const parsedNumbers = keys.replace(/\d+/g, match =>
      String(match)
        .split("")
        .map(digit => `[${digit}]`)
        .join("+")
    );
    const parsedStrings = parsedNumbers.replace(/"([^"]+)"/g, (match, string) =>
      string.split("").join("+")
    );
    const tokenTree = parsedStrings.split(/([+!\[\]()])/g).map(part => {
      ...
      if (this.primitives[part]) return this.primitives[part];
      return this.tokenize(part);
    });

    const tokens: Token[] = Utilities.flat(tokenTree);
   ...
   */
  private tokenize(code: string): Keyword[] {
   ...
   */
  private stringify(tokens: Keyword[]) {
    const string = [];
   ...
   */
  public simplify(code: string, autoEval: boolean = false) {
    const output = this.stringify(this.tokenize(code));
    if (!autoEval) return output;

    // generate a Keyword for the eval function
    const evalToken: Keyword = this.addToken(
      "eval",
